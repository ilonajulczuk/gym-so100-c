            ...
            if mean_reward > self.best_mean_reward:
                ...
                if self.best_model_save_path:
                    ...
                    model_path = os.path.join(
                        ...
                        f"{self.prefix}_best_model_{self.num_timesteps}_steps",
                    )
                    ...
                    if (
                        ...
                        replay_buffer_path = os.path.join(
                            self.best_model_save_path,
                            f"{self.prefix}_best_model_{self.num_timesteps}_steps_replay_buffer.pkl",
                        )
                    ...
                    if self.model.get_vec_normalize_env() is not None:
                        vec_normalize_path = os.path.join(
                            self.best_model_save_path,
                            f"{self.prefix}_best_model_{self.num_timesteps}_steps_vecnormalize.pkl",
                        )
                ...
                if self.video_folder and video_frames:
                    best_video_filename = (
                        f"{self.prefix}_best_model_reward_{mean_reward:.2f}"
                    )
...
def load_checkpoint(
    checkpoint_path, vec_env_stats_path, replay_buffer_path, vec_env, log_dir
):
    ...
    # Load the model
    # model = SAC.load(checkpoint_path, env=vec_env, device=device)

    old_model = SAC.load(checkpoint_path, env=vec_env, device=device)

    # Create new model with desired hyperparameters
    new_model = SAC(
        policy="MultiInputPolicy",
        env=vec_env,
        learning_rate=1e-4,
    ...
    # Transfer the policy and critic weights
    new_model.policy.load_state_dict(old_model.policy.state_dict())
    new_model.critic.load_state_dict(old_model.critic.state_dict())
    new_model.critic_target.load_state_dict(old_model.critic_target.state_dict())
...
class StageBasedTraining:
    ...
    def train(self):
        ...
        if current_steps < self.stage1_end:
            ...
            print(
                f"Stage 1: Exploration phase (continuing from step {current_steps}, {remaining_stage1} steps remaining)"
            )
        ...
        if current_steps < self.stage2_end:
            ...
            print(
                f"Stage 2: Balanced phase (continuing from step {current_steps}, {remaining_stage2} steps remaining)"
            )
        ...
        if current_steps < self.stage3_end:
            ...
            print(
                f"Stage 3: Exploitation phase (continuing from step {current_steps}, {remaining_stage3} steps remaining)"
            )
...
def train_model(
    ...
    if checkpoint_path and os.path.exists(checkpoint_path):
        model, vec_env, start_steps = load_checkpoint(
            checkpoint_path, vec_env_stats_path, replay_buffer_path, vec_env, log_dir
        )
...
def list_available_checkpoints(
    checkpoint_dir="./checkpoints/", prefix="sac_so100_get_cube_new"
):
    ...
    for file in os.listdir(checkpoint_dir):
        if file.startswith(prefix) and file.endswith(".zip"):
            ...
            if match:
                steps = match.group(1)
                vec_stats_file = f"{prefix}_vec_normalize_stats_{steps}.pkl"
    ...
    if checkpoints:
        ...
        for cp in sorted(
            checkpoints, key=lambda x: int(x["steps"]) if x["steps"].isdigit() else 0
        ):
            print(f"  Steps: {cp['steps']}")
            print(f"    Model: {cp['checkpoint']}")
            print(
                f"    VecNormalize: {cp['vec_stats'] if cp['vec_stats'] else 'Not found'}"
            )
            print()
    else:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Train SAC model with optional checkpoint loading"
    )
    parser.add_argument(
        "--checkpoint",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to replay buffer file (e.g., './checkpoints/replay_buffer_pixels_agentpos.pkl')",
    )
    parser.add_argument(
        "--steps", type=int, default=0, help="Start training steps (default: 0)"
    )
    parser.add_argument(
        "--num_envs",
        type=int,
    ...
    parser.add_argument(
        "--prefix", type=str, default=DEFAULT_PREFIX, help="Prefix for the model name"
    )

    parser.add_argument(
        "--task", type=str, default=DEFAULT_TASK, help="Task for the model"
    )
    parser.add_argument(
        "--save-freq",
        type=int,
    ...
    if args.checkpoint and not args.vec_env_stats:
        ...
        if match:
            steps = match.group(1)
            args.vec_env_stats = (
                f"outputs/checkpoints/{args.prefix}_vecnormalize_{steps}_steps.pkl"
            )
            print(f"Auto-detected VecNormalize stats path: {args.vec_env_stats}")
    if args.checkpoint and not args.replay_buffer:
        # Extract step number from checkpoint filename
        import re
        ...
        if match:
            steps = match.group(1)
            args.replay_buffer = (
                f"outputs/checkpoints/{args.prefix}_replay_buffer_{steps}_steps.pkl"
            )
            print(f"Auto-detected Replay Buffer path: {args.replay_buffer}")
