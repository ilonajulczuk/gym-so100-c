...
def convert_episodes_to_trajectories(episodes):
    ...
    for i, episode in enumerate(episodes):
        ...
        episode_infos = infos[:min_length] if len(infos) >= min_length else [{}] * min_length
...
def train_bc_model(trajectories, env, save_path="bc_model"):
    ...
    if len(transitions) == 0:
        raise ValueError("No transitions available for training. Check your demonstration data.")
    ...
    bc_trainer.train(n_epochs=50)  # Adjust epochs as needed
...
def evaluate_model(bc_trainer, env, n_episodes=5):
    """
    ...
    for episode in range(n_episodes):
        ...
        print(f"Episode {episode + 1}: {episode_reward:.2f} reward, {step_count} steps")
...
def main():
    parser = argparse.ArgumentParser(description="Train BC model from teleoperation demonstrations")
    parser.add_argument(
        "--demonstrations",
        type=str,
